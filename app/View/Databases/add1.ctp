<?php
$this->assign('title', '企画申請 | 数理技術相談データベース');
?>

<?php echo $this->Form->create(null, array('type' => 'post', 'url' => 'add1')); ?>
	<div class="container">
		<h2>企画申請</h2>
		
		<ul class="page-navi">
			<li class="active">企画の概要</li>
		...
		<p>登録内容の編集を行う為のアカウントを発行します。</p>
		<table class="table table-bordered table-sm">
			<tr>
				<th class="bg-light w-25">ログインID<span class="text-danger">*</span></th>
				<td>
					<?php echo $this->Form->input('Event.username', array('type' => 'text', 'div' => false, 'label' => false, 'class' => 'form-control form-control-sm')); ?>
					<span class="text-danger small">＊半角英数、-（ハイフン）、_（アンダーバー）のみ使用可。</span>
				</td>
			</tr>
			<tr>
				<th class="bg-light">パスワード<span class="text-danger">*</span></th>
				<td><?php echo $this->Form->input('Event.password', array('type' => 'password', 'div' => false, 'label' => false, 'class' => 'form-control form-control-sm')); ?></td>
			</tr>
			<tr>
				<th class="bg-light">パスワード（確認用）<span class="text-danger">*</span></th>
				<td><?php echo $this->Form->input('Event.password_conf', array('type' => 'password', 'div' => false, 'label' => false, 'class' => 'form-control form-control-sm')); ?></td>
			</tr>
		</table>
		<br>
		...
		<table class="table table-bordered table-sm">
			...
			<tr>
				...
				<td>
					<?php echo $this->Form->input('Event.start', array('type' => 'text', 'div' => false, 'label' => false, 'class' => 'form-control col-3 d-inline datepicker form-control-sm', 'error' => false)); ?>～
					<?php echo $this->Form->input('Event.end', array('type' => 'text', 'div' => false, 'label' => false, 'class' => 'form-control col-3 d-inline datepicker form-control-sm', 'error' => false)); ?>
					<?php echo $this->Form->error('Event.start', array()); ?>
					<?php echo $this->Form->error('Event.end', array()); ?>
				</td>
			</tr>
			<tr>
					...
					<div class="keyword-area mb-2">
						...
						if ( empty($this->request->data['EventKeyword']) )
						{
							for ( $i=0; $i<3; $i++ )
							{
								...
								echo $this->Form->input('EventKeyword.' . $i . '.title', array('type' => 'text', 'div' => false, 'label' => false, 'class' => 'form-control col-3 mb-2 d-inline data-keyword form-control-sm'));
								echo '&nbsp;';
							}
						}
						else
						{
							foreach ( $this->request->data['EventKeyword'] as $key => $event_keyword )
							{
								...
								echo $this->Form->input('EventKeyword.' . $key . '.title', array('type' => 'text', 'div' => false, 'label' => false, 'class' => 'form-control col-3 mb-2 d-inline data-keyword form-control-sm', 'error' => false));
								echo '&nbsp;';
							}
						}
						echo $this->Form->error('EventKeyword.0.title');
						?>
					</div>
