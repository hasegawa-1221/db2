...
class Event extends AppModel {
		...
		if ( $page == 1 )
		{
			...
			$validate = array(
				...
				'password' => array(
					...
					'comparePass' => array(
						'rule' => 'comparePass',
						'message' => 'パスワードとパスワード（確認用）の入力内容が違います',
					),
				),
				'title' => array(
					'notBlank' => array(
				...
				'start' => array(
					...
					'compareTime' => array(
						'rule' => 'compareTime',
						'message' => '開催時期は開始日が終了日より前の日付をお選びください。',
					),
				),
				'end' => array(
					'date' => array(
		...
		elseif ( $page == 2 )
		{
			...
			$validate = array(
				...
				'prepare' => array(
					'between' => array(
						'rule' => array('lengthBetween', 1, 1000),
						'message' => 'これまでの準備状況は1,000文字以内で入力して下さい。'
					)
				),
				'follow' => array(
					'between' => array(
	...
	{
		if ( $this->data['Event']['password'] !== $this->data['Event']['password_conf'] )
		{
			return false;
		}
		return true;
